...
as $k$-mers whose occurrence in \R{simulatedSeq} is associated with
...
which $\hat{y}_i$ spikes up to 1, corresponding to the the ranges
$i \in [1458,1463]$, $i \in [2256,2258]$, and $i \in [5862,5865]$ which
make up the values in \R{peaks\$i}.
...
parameter is discussed in more detail in section \ref{sec:windgini},
but the basic idea is to filter likely false positive suffix array
index positions $i$ out of consideration regardless of their smoothed
scores $\hat{y}_i$ (these likely false positive indices $i$ come from
excessive repetition of the same input sequences contributing
repeatedly to the smoothing windows centered on $i$).
...
would not have been able to detect the \R{"CATACTGAGA"} motif.
...
in the SArKS workflow is, as it seems to specify the \R{halfWindow} ad
...
As suggested in section \R{sec:spatial} above, this can be countered
...
results using any of the specified parameter setttings, and are thus a
...
variation both in length and composition of correlative sequence
...
## showMatch=TRUE includes column specifying exactly what subsequence of
##                of simulatedSeqs registered as a hit;
...
the reverse-complement of $k$-mer \texttt{"TTCCAGC"}, while
...
Low values of the Gini impurity values $g_i$ are use by SArKS to
...
$y_b$, positions $i$ for which the Gini impurity $g_i$ is particularly
far below the maximum value of 1, the smoothed scores will tend to be
more extreme (high or low) than for other positions. Hence a high
...
heuristic parameter whose value is to be assessed empirically via
permutation testing.
...
$i$ with $g^{(\alpha)}_i < g^{(\alpha)}_{\text{min}}$ from either
permutational analysis or $\hat{y}_i$ peak calling if so
